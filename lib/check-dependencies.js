...
const checkDependenciesHelper = (syncOrAsync, config, callback) => {
    ...
    const checkPackage = pkg => {
        ...
        if (options.checkGitUrls) {
            if (/\.git.*#v?(.+)$/.test(versionString)) {
              versionString = (/#v?(.+)$/.exec(versionString))[1];
              if (!semver.valid(versionString)) {
                  return;
              }
            } else if (versionString.startsWith('github:')) {
        ...
        const depVersion = !isSha ? require(depJson).version : require(depJson)._from.split('#')[1];
        if (semver.satisfies(depVersion, versionString) || depVersion === versionString) {
            log(
